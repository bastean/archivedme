...
export const BookingProvider = ({ children }) => {
	...
	const { headersConfig, ClientAxios, jwtokenName, alert, setAlert, alertOut, alertRunning } =
		useAuthProvider();

	const autoCloseForm = (showForm) => {
		setTimeout(() => {
			document.querySelector('#bookingForm').classList.add('bookingFormAnimationClose');
			setTimeout(() => {
				showForm(false);
			}, 900);
		}, 2250);
	...
	const registerBookingOnDB = async (bookingToRegister, showForm) => {
		if (localStorage.getItem(jwtokenName)) {
			try {
				const { data } = await ClientAxios.post('/booking', bookingToRegister, headersConfig);
	...
	const deleteBookingOnDB = async (bookingToDeleteID, showForm) => {
		if (confirm('Delete Booking?')) {
			if (localStorage.getItem(jwtokenName)) {
				try {
					const { data } = await ClientAxios.delete(`/booking/${bookingToDeleteID}`, headersConfig);
