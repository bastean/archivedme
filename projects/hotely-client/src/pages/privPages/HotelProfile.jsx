...
const HotelProfile = () => {
	...
	const { hotel, editProfile, changePassword, setAlert, alertOut, alertRunning } =
		useAuthProvider();
	...
	const handleSubmit = (e) => {
		...
		if (showForm === 'edit-profile') {
			if ([profile.name, profile.email, profile.phone, profile.web].includes('')) {
			...
			if ([password.currentPwd, password.newPwd, password.confirmPwd].includes('')) {
				setAlert({ error: true, msg: 'Empty Fields' });
				alertOut();
				return;
			}
